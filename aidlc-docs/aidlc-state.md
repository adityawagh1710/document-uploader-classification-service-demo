...
- [ ] **Deploy workflow continues to fail on push-to-main** with `Could not assume role with OIDC: No OpenIDConnect provider found in your account`. Root cause is the unexecuted operator hand-off (placeholder account IDs in `infra/config/*.ts` + no OIDC provider in any AWS account + no IAM role with trust policy + no `cdk bootstrap` + no SNS topic at SSM `/observability/sns-topic-arn/<env>` + no `AWS_DEPLOY_ROLE_<ENV>` GitHub secret). `chore/disable-deploy-workflow` branch was pushed as a way to mute the auto-trigger but **deliberately not merged** — the `push:` trigger stays active so the failed deployments at `/deployments/dev` remain visible as a forcing function for prioritising the hand-off
